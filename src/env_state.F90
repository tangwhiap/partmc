...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !> Computes saturated vapor pressure (Pa) with respect to water using the
  !> IceGrowth condensation parameterization.
  real(kind=dp) function env_state_saturated_vapor_pressure_wrt_water_2(T)
    ...
    env_state_saturated_vapor_pressure_wrt_water_2 = const%water_eq_vap_press &
         * 10d0**(7.45d0 * (T - const%water_freeze_temp) / (T - 38d0))
